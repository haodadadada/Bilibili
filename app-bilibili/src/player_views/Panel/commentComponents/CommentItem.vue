    ...
    <div class="comments" v-if="props.commentList">
        <div class="comment" v-for="comment of props.commentList" :key="comment.rpid">
            <div class="header mb-10">
                ...
                <div class="flex-column">
                    <div class="flex-start">
                        <div class="size-14 mr-5" :class="comment.member.vip.vipType === 0 ? 'member-name-notvip' : 'member-name-vip'">{{ comment.member.uname }}</div>
                        <component 
                        ...
                        <component :is="upIcon" class="member-icon" v-if="props.upper && 'mid' in props.upper && comment.member.mid == props.upper.mid" style="width: 24px; height: 24px;"></component>
                    </div>
                    <div class="pubdate size-12">{{ formatTime(comment.ctime) }}</div>
                </div>
            </div>
            <div class="comment-content size-15 mb-10" v-clamp="({isExtend: comment.isExtend, lineCount: 5})">
                <i v-if="'isTop' in comment && comment.isTop" id="top">置顶</i>
                <div class="mb-10 size-14 text" v-html="finalHTML(comment.content.message, comment.content.members.map(item => item.uname), comment.content.emote)"></div>
                <div class="pic"></div>
            ...
            <div class="comment-info flex-start mb-10">
                ...
                <span class="size-13 mr-10">{{ comment.like }}</span>
            ...
            <div class="comment-replies" v-if="props.isNeedShowReplies && comment.replies.length > 0">
                <div class="reply-item max-line size-14 line-15 mb-5" v-for="reply of comment.replies" :key="reply.rpid">
                    <span class="title">{{ reply.member.uname }}</span>
                    <component :is="upIcon" class="member-icon img" v-if="props.upper && 'mid' in props.upper && reply.member.mid == props.upper.mid" style="width: 24px; height: 24px;"></component>
                    <span> : </span>
                ...
                <span class="extend-btn size-12" @click="sendExtendReplies({oid: comment.oid, rpid: comment.rpid})">{{ comment.reply_control.sub_reply_entry_text }}</span>
            </div>
        </div>
        ...
        if(emote) {
            Object.keys(emote).forEach(key => {
                atImgStr = R.replace(
                    ...
                    emote[key].meta.size === 1 ? `<img width="20" height="20" src=${emote[key].url} style="vertical-align: bottom;" alt="" crossorigin="anonymous">` : `<img width="50" height="50" src=${emote[key].url} style="vertical-align: bottom;" alt="" crossorigin="anonymous">`,
                    atImgStr
                );
            });
        };
