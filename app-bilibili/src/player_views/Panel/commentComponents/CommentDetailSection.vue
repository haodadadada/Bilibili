<template>
    ...
    <section class="content" id="single-comment" v-if="!isLoadingDetailComment">
        <div class="detail-header flex-start">
            ...
            <span class="size-15">详情</span>
        </div>

        <CommentItem :commentList="rootComment ? [rootComment] : null" :isNeedShowReplies="false"></CommentItem>

        <div class="line mr-10"></div>
        <div class="sub-reply-title size-13 mt-10">{{ rootComment ? rootComment.reply_control.sub_reply_title_text : ''}}</div>

        <CommentItem :commentList="replyList" :isNeedShowReplies="false"></CommentItem>
        <div id="sentinel"></div>
        <div v-if="isEndDetail" class="nomore-comment text-center size-13">没有更多评论</div>
    </section>
</template>
...
<style lang="scss" scoped>
    ...
    .content {
        ...
        color: #fff;
        overflow-y: scroll;
        height: calc(100vh - var(--nav-height) - $tabHeight);
