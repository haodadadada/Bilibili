    ...
    <div class="container-rcmd">
        <div class="content-rcmd">
            <div class="content-wrap" v-if="rcmdVideos.length > 0">
                <div v-for="(item, index) in rcmdVideos" :key="item.id" class="box-card" @click="handleClickVideo(item, index)">
...
<script setup lang="ts">
    import { defineEmits, ref, nextTick, onActivated, onDeactivated } from 'vue';
    ...
    // 获取推荐视频
    const fetchHomeRecommand = throttle(async (sessdata: string, freshIndex: number, lastShowList: string[], ps = pageCount): Promise<void> => {
    ...
    const refreshRcmdVideo = async (): Promise<void> => {
        ...
        resetRcmdVideos();
        try {
            await fetchHomeRecommand(sessdata, freshIdx.value, parseLastShowList(lastRcmdVideos.value));
        } catch (err) {
    ...
    let curPlayerWindowIndex = ref<number>(-1);  // 当前播放器窗口的索引
    ...
    const openPlayerWindow = (videoItem: RcmdVideoItem, index: number, routePath: string): void => {
        ...
        const jsonData = JSON.stringify({ aid: id, bvid });
    ...
    onActivated(async (): Promise<void> => {
        ...
        observer = observeLastEle(sentinel, {}, fetchHomeRecommand, sessdata, freshIdx.value, parseLastShowList(lastRcmdVideos.value));
    });
