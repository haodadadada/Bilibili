...
<script setup lang="ts">
    import { defineEmits ,ref, onActivated, onDeactivated, nextTick } from 'vue';
    ...
    const fetchPopularVideos = async ({ sessdata, pn = 1, ps = 20 }: { sessdata: string; pn?: number; ps?: number }) => {
        if (!isNoMore.value) {
    ...
    const fetchHomePopularPlayurl = R.curry(async (item: PopularVideoItem, sessdata: string): Promise<VideoDashItem[] | null> => {
        const { bvid, cid } = item;
    ...
    onActivated(async () => {
        await fetchPopularVideos({ sessdata });
        await nextTick();
        const sentinel = document.getElementById('sentinel');
        if (sentinel) {
            observer = observeLastEle(sentinel, {}, fetchPopularVideos, { sessdata, pn: freshIdx.value });
        };
    });
