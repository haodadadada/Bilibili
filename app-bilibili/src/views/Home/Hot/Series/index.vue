<template>
    <div>
        <section class="content-hot" v-if="!isLoading">
            <div class="cards-header flex-center">
                <div class="series-header flex-center">
                    <span class="size-12 mr-10 tip-header">每周五晚18:00更新</span>
                    <span class="current-period mr-10 size-14 flex-center" :class="isLight ? '' : 'current-period-dark'">
                        ...
                        <div class="panel" :class="isShowPanel ? 'visiable' : ''">
                            <div class="panel-select-item  flex-column-start" :class="seriesPeriodNth === item.number ? 'panel-selected' : ''" v-for="item of seriesList" :key="item.number" @click="selectSeries(item.number)">
                                <div class="size-14 panel-title">{{ item.subject }}</div>
                                <div class="size-12 panel-info">{{ item.name }}</div>
                            </div>
                        </div>
                    </span>
                    <span class="play-btn size-15 flex-center">
                ...
                <div class="box-card flex-column-center">
                    ...
                    <div class="card-info">
                        ...
                        <div class="card-reason mt-5 flex-center flex-start">
                            ...
                            <span class="size-12">{{item.rcmd_reason}}</span>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="no-more size-12">已经到底啦</div>
    </div>
</template>

<script setup lang="ts">
    import { defineEmits, ref, onMounted, nextTick, computed, onDeactivated, onActivated } from 'vue';
    ...
    const openPlayerWindow = (videoItem: SeriesVideoItem, index: number, routePath: string): void => {
        ...
        const jsonData = JSON.stringify({ aid, bvid });
