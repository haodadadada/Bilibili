        ...
        <div ref="contentRef">
            ...
            <section class="content-search">
                <div class="content-wrap" v-if="searchVideos.length > 0 && !isLoading">
                    <div v-for="(item, index) in searchFilterVideos" :key="item.id" class="box-card" @click="openPlayerWindow(item, index)">
                        <div class="box-card-wrap flex-column-center" v-if="item && item.id !== 0">
...
<script setup lang="ts">
    ...
    const handleMouseEnter = async (item: SearchVideoItem, index: number): Promise<void> => {
        ...
        try {
            if(!('cid' in item)) {
                const response = await getHomeVideoInfo({bvid: item.bvid}, sessdata);
                const successAction = R.pipe(
                    R.pathOr(-1, ['data', 'cid']),
                    R.tap((cid: number) => {
                        item.cid = cid;
