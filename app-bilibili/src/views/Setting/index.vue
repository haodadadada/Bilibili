    ...
    <div class="container">
        <header class="owner flex-start" v-if="isLogin">
            ...
            <div class="right flex-column-between">
                <div class="user flex-start">
                    <span class="size-16 weight-6 mr-5">{{ userInfo.uname || '' }}</span>
                    <div v-if="levelIcon">
                ...
                <div class="exit-login flex-start" @click="handleClickExitBtn">
                    ...
                    <span class="size-14">退出账号登录</span>
                </div>
            </div>
        </header>
        <header class="login flex pt-20 pb-20" v-else>
            <div class="login-btn radius-7 size-13" @click="handleClickLoginBtn">立即登录</div>
        </header>
        <div class="line"></div>
        <section class="content">
            <div class="common">
                ...
                <div class="part">
                    <div class="subtitle size-15 weight-5">字体选择：（若字体显示异常，可切换为默认字体）</div>
                    <el-select 
                        v-model="fontIndex" 
                ...
                <div class="start-automatically flex-start part">
                    <div class="start-automatically-wrap flex-start">
                        ...
                        <span class="size-14">开机时自启动</span>
                    </div>
                </div>
                <div class="part">
...
<script setup lang="ts">
    ...
    const isLogin = store.state.userModule.isLogin || false;
    ...
    if('current_level' in userInfo) {
        const module = images[levelIconsPath[userInfo.current_level]];
        // markRaw不需要对组件进行深度监听
    ...
    const handleChangeFontChoice = (index: number) => {
        ...
        const data = JSON.stringify(configData.value);
        window.electronAPI.sendMessage('main_set_setting', data);
        window.electronAPI.sendMessage('update_font', fontIndex.value);
    };
