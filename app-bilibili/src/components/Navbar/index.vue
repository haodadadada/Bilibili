        ...
        <div class="main mr-10">
            <div class="nav-items">
                ...
                <!-- <Transition name="opacity-fade"> -->
                    <!-- <div class="nav-btn" v-if="navStatus"> -->
                        <div v-for="item of props.navItems" :key="item.path" @click="navigateTo(item.path)" :class="{'nav-active': activePath.includes(item.path)}" class="nav-item size-15">
                            <div>{{ item.name }}</div>
            ...
            <div class="search">
                <input v-model="keyword" type="text" class="search-ipt size-12" placeholder="搜索感兴趣的视频" @keydown="handleSearchKeyDown">
            </div>
        </div>
...
<script setup lang="js">
    ...
    const updateNavStatus = statue => navStatus.value = statue;
    let keyword = ref('');
    // route.path 初始值为 '/'
    ...
    const handleSearchKeyDown = async (event) => {
        if (event.key === 'Enter') {
            console.log('输入确认', event.target.value);
