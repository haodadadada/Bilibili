    ...
    <div class="container" @mouseleave="handleMouseLeave($event, '.primary-menu')">
        ...
        <div class="content-data primary-menu">
            <div v-for="(item, index) of props.list" :key="item.text" class="drop_item" @click="chooseData($event, [index])" @mouseover="handleMouseOver($event, '.secondary-menu' + index, index)"
                ...
                {{ item.text }}
                <div v-if="'nextChild' in item" class="content-data-child" :class="'secondary-menu' + index">
...
const handleMouseOver = (event, className, index) => {
    if(event) {
        event.stopPropagation;
    }
    let targetEle = document.querySelector(className);
    if(!targetEle) {
        return;
    }
...
const handleMouseLeave = (event, className) => {
    if(event) {
        event.stopPropagation;
    }
    let targetEle = document.querySelector(className);
...
onMounted(async () => {
    try {
        await nextTick();
        itemHeight = document.querySelector('.drop_item').getBoundingClientRect().height;
    }
    catch(err) {
